...
async function flockPromise(
  ...
  //
  // On wasm32 (the browser guest) there is no native binding at all, and merely
  // attempting the dynamic import throws uncatchably while the runtime links the
  // CJS module graph — so short-circuit on arch before importing. Advisory
  // cluster locking then isn't available, but a single-process runtime has no
  // competing postmaster to guard against, so degrade to a no-op. Multi-process
  // hosts that ship the addon keep real flock.
  const arch =
    (globalThis as { Deno?: { build?: { arch?: string } } }).Deno?.build
      ?.arch ?? (globalThis as { process?: { arch?: string } }).process?.arch
  if (arch === 'wasm32') return
